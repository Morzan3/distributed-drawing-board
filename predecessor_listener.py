        ...
        while not self._stop_event.is_set():
            try:
                ...
                while len(message_size) < 8:
                    packet = self.connection_data['connection'].recv(8 - len(message_size))
                    if not packet:
                        return None
                    message_size += packet
                ...
                logger.error(ex)
                raise ex
        logger.info("Prdecessor listener connection shutdowned ")
    ...
    def handle_message(self, parsed_message):
        ...
        if message_type == events.EventType.DRAWING_INFORMATION:
            self.event_queue.put(
                events.DrawingInformationEvent(data['client_uuid'], data['timestamp'], data['points'],
                                        data['color'])
            )
